  ...
  output$odHeatmap <- renderPlotly({
    ...
    od_workers_plotly <- ggplot(
      ...
      aes(
        ...
        text = paste0(
          "There are ", formattable::comma(N_workers, digits = 0), " workers \n",
  ...
  output$alluPlot <- renderPlot(
    {
      ...
      dataset_simlabel_selected_allu <- dataset_simlabel %>%
        mutate(
          journey_type_colour_highlight = 
            ifelse((origin_ID %in% input$residence_group & destination_ID %in% input$work_group),
                   journey_type_colour,
                   "#cccccc33"
                   ),
        ) %>%
        mutate(
          ...
          journey_type_colour_highlight = 
            factor(journey_type_colour_highlight,
                   levels = c("#4daf4a66", "#377eb866", "#ff7f0066", "#984ea366", "#cccccc33"))
        )

      OD_work_alluvial_travelcolour <-
        ggplot(
          dataset_simlabel_selected_allu,
